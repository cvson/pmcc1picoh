...
int main(int argc, char *argv[])
{
    ...
    //string fnameout = "../outputs/mcdata_sample_newbin.root";
    ...
    FitSample sam6(5, "CRIICHiResPID",v_pedges, v_cthedges,tdata);
    sam6.SetNorm(potData/potMC);
    samples.push_back(&sam6);
    
    FitSample sam7(6, "CRIICLoResPID",v_pedges, v_cthedges,tdata);
    sam7.SetNorm(potData/potMC);
    samples.push_back(&sam7);
    
    FitSample sam8(7, "M2TRKHiResPID",v_pedges, v_cthedges,tdata);
    sam8.SetNorm(potData/potMC);
    samples.push_back(&sam8);
    
    FitSample sam9(8, "M2TRKLoResPID",v_pedges, v_cthedges,tdata);
    sam9.SetNorm(potData/potMC);
    samples.push_back(&sam9);
    
    FitSample sam10(9, "M2TRKAllElse",v_pedges, v_cthedges,tdata);
    sam10.SetNorm(potData/potMC);
    samples.push_back(&sam10);
    
    
    FitSample sam11(10, "ONETRKLoQsq",v_pedges, v_cthedges,tdata);
    sam11.SetNorm(potData/potMC);
    samples.push_back(&sam11);
    
    FitSample sam12(11, "ONETRKHiQsq",v_pedges, v_cthedges,tdata);
    sam12.SetNorm(potData/potMC);
    samples.push_back(&sam12);
    
    //Make it Manually
    const char *bsnameTag[12] = {"SIG", "CRI", "CRIIALoQsq", "CRIIAHiQsq","CRIIB", "CRIICHiResPID", "CRIICLoResPID",  "M2TRKHiResPID","M2TRKLoResPID", "M2TRKAllElse", "ONETRKLoQsq","ONETRKHiQsq"};
    ...
    for(size_t s=0;s<samples.size();s++){
        ((FitSample*)(samples[s]))->FillEventHisto(2);
