...
class baseTreeWeight {
   ...
   // virtual Int_t    CutCCQEPID2trk(Long64_t entry);
    ...
    virtual void     checkWeight(TString ipFile, char* treeName, TString opFile);
...
baseTreeWeight::baseTreeWeight(TTree *tree) : fChain(0)
{
    ...
    if (tree == 0) {
        ...
        TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("../../datafsipionFS/merged_ccqe_forResponseFunction_pionFS.root");
        if (!f || !f->IsOpen()) {
            f = new TFile("../../datafsipionFS/merged_ccqe_forResponseFunction_pionFS.root");
        }
...
Int_t baseTreeWeight::CutPreselection(Long64_t entry)
{
    ...
    //if (Ntrack>1 && pang<90 && !(mufc==0 && pene<9)) return 1;//version of no cut on Ntrack
     else return -1;
}

Int_t    baseTreeWeight::CutSignalR(Long64_t entry){
    if (CutPreselection(entry)>0 && pidfsi>0.05 && veract*7.6634e-2<50) {//opt1, opt2, final
        return 1;
    }
    else return -1;
}
Int_t    baseTreeWeight::CutCR1(Long64_t entry){
    if (CutPreselection(entry)>0 && Ntrack==2 && pidfsi>0.0 && veract*7.6634e-2>50 && veract*7.6634e-2<100) {//opt9, final
        return 1;
    }
    else return -1;
}
Int_t    baseTreeWeight::CutCR23Comb(Long64_t entry){
    if (Ntrack>2 && pang<90 && pidfsi>0.0 && !(mufc==0 && pene<9) && veract*7.6634e-2>50 && veract*7.6634e-2<100) {//final
        return 1;
    }
    else return -1;
}
...
//need to match with xsecparameter:SampleTypes
...
//matching with xsecparameter:ReactionTypes
Int_t baseTreeWeight::CutInteractionbyIDnew(Long64_t entry, Int_t myid)
{
    ...
    //CC0pi
        ...
        if (npioncount==0 && abs(inttype)<30 && abs(inttype)!=16) return 1;
        else return -1;
    }
    //CC1pi
    else if (myid==2){
        if (npioncount==1 && abs(inttype)<30 && abs(inttype)!=16) return 1;
        else return -1;
    }
    //CCNpi
    else if (myid==3){
...
Int_t    baseTreeWeight::GetSampleType(Long64_t entry){
    ...
    else if (CutPreselection(entry)>0 && pidfsi>0.05 && veract*7.6634e-2>=50 && muang_t<30) return 1;
    
    else if (CutPreselection(entry)>0 && pidfsi<=0.05 && muang_t<30 && mumucl>0.6 && pmucl>0.25) return 2;

    else if (CutPreselection(entry)>0 && pidfsi<=0.05 && muang_t<30 && mumucl>0.6 && pmucl<=0.25) return 3;
	else return -1;
