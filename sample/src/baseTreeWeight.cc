...
void baseTreeWeight::checkWeight(TString ipFile, char* treeName, TString opFile)
{
    ...
    //TODO: Binning scheme need to be optimized
        ...
        for (Int_t ipara=0; ipara<NPARAMETER; ipara++) {
            for (Int_t ivari=0; ivari<NVARIATION; ivari++) {
                hreweight[ipara][ivari] = new TH1F(Form("hweight_para%d_sigma%d",ipara,ivari),"",500,-50,50);
                for (Int_t iitype=0; iitype<NINTERACTION; iitype++) {
                    henmc[ipara][ivari][iitype] = new TH1F(Form("henmc_para%d_sigma%d_type%d",ipara,ivari,iitype),"",200,0,10);
                }
            }
        }
    ...
    for (Long64_t jentry=0; jentry<nentries;jentry++) {
        ...
        nb = fChain->GetEntry(jentry);   nbytes += nb;
                ...
                for (Int_t ipara=0; ipara<NPARAMETER; ipara++) {
                    for (Int_t ivari=0; ivari<NVARIATION; ivari++) {
                        paraIndex = ipara*7+ivari;
                        if(reweight[paraIndex]>-100&&reweight[paraIndex]<100) {
                          hreweight[ipara][ivari]->Fill(reweight[paraIndex]);
                          for (Int_t iitype=0; iitype<NINTERACTION; iitype++) {
			if(CutInteractionbyIDnew(ientry,iitype)>0) henmc[ipara][ivari][iitype]->Fill(nuE,reweight[paraIndex]);   
			 }
                          }
                    }//end ivari
                }//end for ipara
