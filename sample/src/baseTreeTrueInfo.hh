...
class baseTreeTrueInfo {
	...
	virtual Int_t    CutFV(Long64_t entry);
    ...
    virtual void     MakeBasicHistoBySample(TString ipFile, char* treeName, TString opFile, Int_t iSample, Bool_t isGENIE=false);
...
Int_t baseTreeTrueInfo::CutFV(Long64_t entry)
{
    if (abs(xnu)<50 && abs(ynu)<50 && znu>-152 && znu<-90) return 1;
    else return -1;
}
...
//GENIE
Int_t baseTreeTrueInfo::CutInteractionbyIDGENIE(Long64_t entry, Int_t interactID){
