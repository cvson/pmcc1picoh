...
baseTree::baseTree(TTree *tree) : fChain(0)
{
    ...
    if (tree == 0) {
        TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("../../datafsi/datamc_merged_ccqe_addpidFFnew.root");
        if (!f || !f->IsOpen()) {
            f = new TFile("../../datafsi/datamc_merged_ccqe_addpidFFnew.root");
        }
...
Int_t baseTree::CutTopologybyID(Long64_t entry, Int_t topoID)
{
    ...
    //CRI
        ...
        if (CutCR1(entry)>0 || CutMore2TrackHiResPID(entry)>0) return 1;//cr1 optimized
        else return -1;
    }
    //CRII
    else if (topoID==5) {
        if (CutCR23QEFreedLowVeract(entry)>0 || CutMore2TrackLoResPID(entry)>0) return 1;//cr2-A: CCQ isolated
        else return -1;
    }
    //CRIII
    else if (topoID==6) {
        if (CutCR23QEFreedHighVeract(entry)>0 || CutMore2TrackAllElse(entry)>0) return 1;//cr2-B: CCQ isolated and low veract
        else return -1;
    }
    /////////////////////Newadd 2016/01/20
    //antipid to check
    ...
    //one muon track no side-escaped
    else if (topoID==9) {
        if (Ntrack==1 && mumucl>0.6 && !(mufc==0 && pene<9)) return 1;
        else return -1;
    }
    //one muon stopped
    else if (topoID==10) {
        if (Ntrack==1 && mumucl>0.6 && mufc==1 && pene<9) return 1;
        else return -1;
    }
    //one muon and one pion track all
    else if (topoID==11) {
        if (Ntrack==2 && mumucl>0.6 && pmucl>0.25) return 1;
        else return -1;
    }
    
    //one muon and one pion track all
    else if (topoID==12) {
        if (Ntrack==2 && mumucl>0.6 && pmucl>0.25 && !(mufc==0 && pene<9)) return 1;
        else return -1;
    }
    //one muon and one pion stopped muon
    else if (topoID==13) {
        if (Ntrack==2 && mumucl>0.6 && pmucl>0.25 && mufc==1 && pene<9) return 1;
        else return -1;
    }
    //one muon and one proton track all
    else if (topoID==14) {
        if (Ntrack==2 && mumucl>0.6 && pmucl<0.05) return 1;
        else return -1;
    }
    
    //one muon and one proton track all
    else if (topoID==15) {
        if (Ntrack==2 && mumucl>0.6 && pmucl<0.05 && !(mufc==0 && pene<9)) return 1;
        else return -1;
    }
    //one muon and one proton stopped
    else if (topoID==16) {
        if (Ntrack==2 && mumucl>0.6 && pmucl<0.05 && mufc==1 && pene<9) return 1;
        else return -1;
    }
    //one proton track all
    else if (topoID==17) {
        if (Ntrack==1 && mumucl<0.05) return 1;
        else return -1;
    }
    
    //one proton track all
    else if (topoID==18) {
        if (Ntrack==1 && mumucl<0.05 && !(mufc==0 && pene<9)) return 1;
        else return -1;
    }
    //one proton stopped
    else if (topoID==19) {
        if (Ntrack==1 && mumucl<0.05 && mufc==1 && pene<9) return 1;
        else return -1;
    }
    ...
    //one muon and one pion track all
        ...
        if (Ntrack==2 && pang<90 && pidfsi<=0.08 && mumucl>0.6 && pmucl>0.25 && !(mufc==0 && pene<9)) return 1;
        else return -1;
    }
    //one muon and one pion stopped muon
    else if (topoID==22) {
        if (Ntrack==2 && pang<90 && pidfsi<=0.08 && mumucl>0.6 && pmucl>0.25 && mufc==1 && pene<9) return 1;
        else return -1;
    }
    ...
    //one muon and one pion track all
        ...
        if (CutCCQEKikawa2trk(entry)>0 && !(mufc==0 && pene<9)) return 1;
        else return -1;
    }
    //one muon and one pion stopped muon
    else if (topoID==25) {
        if (CutCCQEKikawa2trk(entry)>0 && mufc==1 && pene<9) return 1;
        else return -1;
    }
    //kikawa-san selection non-side escaping
    else if (topoID==26) {
        if (CutPreselection(entry)>0 && CutNOM(entry)>0) return 1;
        else return -1;
    }
...
Int_t baseTree::CutTopologyFSIbyID(Long64_t entry, Int_t topoID)
{
    ...
    //signal only to check FSI
        ...
        if (CutPreselection(entry)>0 && abs(inttype)==16) return 1;//CCQE with one track
        else return -1;
    }
