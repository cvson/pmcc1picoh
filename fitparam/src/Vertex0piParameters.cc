...
Vertex0piParameters::Vertex0piParameters(const char *fname, const char *name)
{
    m_name    = name;
    hasCovMat = false;
    
    cc0pi_recode = 1; //reaction code for CC0pi in sample/src/baseTree.hh
...
Vertex0piParameters::~Vertex0piParameters()
{;}

void Vertex0piParameters::SetBinning(const char *fname)
{
    ifstream fin(fname);
    assert(fin.is_open());
    string line;
...
int Vertex0piParameters::GetBinIndex(double p, double cth)
{
    int binn = BADBIN;
    for(size_t i=0;i<m_bins.size();i++)
    {
...
// initEventMap
void Vertex0piParameters::InitEventMap(std::vector<AnaSample*> &sample)
{
    m_evmap.clear();
    ...
    for(size_t s=0;s<sample.size();s++)
    {
        ...
        for(int i=0;i<sample[s]->GetN();i++)
        {
            ...
            if(ev->GetReaction() != cc0pi_recode) //pass if not CC1picoh
            {
                row.push_back(code);
                continue;
            }
...
// EventWeghts
void Vertex0piParameters::EventWeights(std::vector<AnaSample*> &sample,
...
void Vertex0piParameters::ReWeight(AnaEvent *event, int nsample, int nevent,
                              std::vector<double> &params)
{
