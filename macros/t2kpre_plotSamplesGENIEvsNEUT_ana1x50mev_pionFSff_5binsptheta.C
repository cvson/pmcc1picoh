{
    ...
    TFile *pfile = new TFile("../outputs/neutvsgenie_sample_ana1x50mepionFS_5binsptheta.root");
   ...
   const char *nameslong[] = {"signal", "cc0#pi", "cc1#pi", "ccother", "nc", "numubar", "nue", "ingrid"}; 
    ...
    for (int ireac=0;ireac<nreac;ireac++){
        if(ireac<4 || ireac==5)leg0->AddEntry(henu_true[0][ireac], nameslong[ireac],"lf");
    ...
    for (int ireac=0;ireac<nreac;ireac++){
        if(ireac<4 || ireac==5)leg0nodt->AddEntry(henu_true[0][ireac], nameslong[ireac],"lf");
    ...
    for(int isam=0;isam<nsample;isam++){
       ...
       leg0->Draw();
        ...
        gPad->Modified();
        gPad->Print(Form("plots/genievsneut_ana1x50mev_pionFS_5binptheta_rec_sam%d.eps",isam));
