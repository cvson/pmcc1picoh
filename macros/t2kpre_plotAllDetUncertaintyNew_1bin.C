{
    ...
    const char *nameSample_long[NSAMPLE]={"Signal sample", "Control I", "Control II","Control III"};
    ...
    const char *subname[NSYS] = {"pevar_1bptheta","birks_1bptheta","reco_1bpthetaall","fvtimeall","noise_1bptheta","hiteff_1bptheta"};
    const char *subnamelong[NSYS] = {"P.E. variation","Birks effect","Track reconstruction","FV + Veto","Noise","Hit efficiency"};
    TFile *paFile[NSYS];
    //paFile[0] = new TFile("../inputs/detresponse_pe_1bptheta_uncertainty.root");
    paFile[0] = new TFile("../inputs/detresponse_pevar_1bptheta_uncertainty.root");
    paFile[1] = new TFile("../inputs/detresponse_birks_1bptheta_uncertainty.root");
    paFile[2] = new TFile("../inputs/detresponse_reco_1bptheta_uncertainty.root");
    paFile[3] = new TFile("../inputs/detresponse_FVTime_1bptheta.root");
    paFile[4] = new TFile("../inputs/detresponse_noise_1bptheta_uncertainty.root");
    paFile[5] = new TFile("../inputs/detresponse_hiteff_1bptheta_uncertainty.root");
    ...
    for (Int_t isys=0; isys<NSYS; ++isys) {
	    ...
	    //else pUncertainty[isys][isam] = (TGraph*)paFile[isys]->Get(Form("detresponse_%s_1bptheta_uncertainty_sam%d",subname[isys],isam));
    ...
    ///////////////////////////////////////////////////////////////////////////////////////
    TFile *popfile = new TFile("../inputs/detectorAll_1bptheta_covariance_matrix.root","RECREATE");
    for (Int_t isam=0; isam<NSAMPLE; ++isam) {
        icolor = TColor::GetColor(colorcode[isam]);
        imarker = markerStyle[isam];
    ...
    for (Int_t isys=0; isys<NSYS; ++isys) {
        ...
        leg0->Draw("same");
        gPad->Modified();
        gPad->Print(Form("plots/alldet_1bptheta_uncertainty_%s_compSample.eps",subname[isys]));
    }
    ...
    leg0->Draw("same");
    gPad->Modified();
    gPad->Print("plots/alldet_1bptheta_alluncertainty_compSample.eps");
