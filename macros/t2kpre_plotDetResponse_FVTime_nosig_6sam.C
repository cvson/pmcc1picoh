{
    ...
    const char *nameSample[] = {"SIG", "CRI", "CRII","CRIII","CRIV","CRV"};//by definition in baseTree:CutSamplebyID
...
const int markerStyleBin[NBINCCQE] = {20, 21, 22, 24, 25/*, 26,27,28,29*/};   
	...
	TLatex* tlx=new TLatex(0.42, 0.95,Form("%s (data) ",nameSample_long[isam]));
        tlx->SetNDC(kTRUE); // <- use NDC coordinate
        tlx->SetTextSize(0.05);
        tlx->Draw();
        gPad->Print(Form("plots/detresponse_fvtimenosignewfv_dt_5bptheta_timing_sam%d.eps",isam));
	...
	tlx->Draw();
        gPad->Print(Form("plots/detresponse_fvtimenosignewfv_dt_5bptheta_fv_sam%d.eps",isam));
        tlx=new TLatex(0.42, 0.95,Form("%s (MC) ",nameSample_long[isam]));
        tlx->SetNDC(kTRUE); // <- use NDC coordinate
        tlx->SetTextSize(0.05);
        new TCanvas;
	...
	tlx=new TLatex(0.42, 0.95,Form("%s (MC) ",nameSample_long[isam]));
        tlx->SetNDC(kTRUE); // <- use NDC coordinate
        tlx->SetTextSize(0.05);
        gPad->Print(Form("plots/detresponse_fvtimenosignewfv_mc_5bptheta_fv_sam%d.eps",isam));
      	//by bin
...
*/
	...
	tlx=new TLatex(0.42, 0.95,Form("%s (data) ",nameSample_long[isam]));
        tlx->SetNDC(kTRUE); // <- use NDC coordinate
        tlx->SetTextSize(0.05);
	tlx->Draw();
        gPad->Print(Form("plots/detresponse_fvtimenosignewfv_dt_5bptheta_veto_sam%d.eps",isam));
	...
	tlx=new TLatex(0.42, 0.95,Form("%s (MC) ",nameSample_long[isam]));
        ...
        tlx->SetTextSize(0.05);
	tlx->Draw();
        gPad->Print(Form("plots/detresponse_fvtimenosignewfv_mc_5bptheta_veto_sam%d.eps",isam));
...
leg0bin->AddEntry(h2,"Sideband","l");
 ...
 new TCanvas;
	...
	pgrccqesub_fv[1][0]->GetYaxis()->SetTitle("Data-MC difference");
        ...
        tlx->Draw();
        gPad->Modified();
        gPad->Print(Form("plots/detresponse_fvtimenosignewfv_5bptheta_sigvssb_%s_compAllBin.eps", "fv"));
...
new TCanvas;
    ...
    tlx->Draw();
    gPad->Modified();
    gPad->Print(Form("plots/detresponse_fvtimenosignewfv_5bptheta_sigvssb_%s_compAllBin.eps", "veto"));
...
/////////////////////////////
    ...
    leg0->Draw("same");
    gPad->Modified();
    gPad->Print("plots/detresponse_fvtimenosignewfv_alldet_5bptheta_fv_compSample.eps");
    ...
    leg0->Draw("same");
    gPad->Modified();
    gPad->Print("plots/detresponse_fvtimenosignewfv_alldet_5bptheta_veto_compSample.eps");
    ...
    leg0->Draw("same");
    gPad->Modified();
    gPad->Print("plots/detresponse_fvtimenosignewfv_alldet_5bptheta_fvveto_compSample.eps");
