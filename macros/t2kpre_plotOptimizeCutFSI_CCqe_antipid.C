{
    ...
    //sum of background
    //7 is after pid<0.0
    TH1F* hsig = (TH1F*)hmcen[7][2]->Clone("hsignal");
    TH1F* hbkg = (TH1F*)hmcen[7][3]->Clone("hbackground");
    for (Int_t iint=4; iint<NINTERACTION; ++iint) {
        hbkg->Add(hmcen[7][iint]);
    }
    hbkg->Add(hmcen[7][1]);//also cccoh  
    //optimization
    ...
    Double_t valsig;
    Double_t valbkg;
    Double_t significantRaw = /*valsigTot/valbkgTot;*/valsigTot/sqrt(valbkgTot);
    ...
    for (int ix = nx; ix >= 1; ix--){
        ...
        significant = /*valsig/valbkg;*/valsig/sqrt(valbkg);
        ...
        cout<<"raw "<<significantRaw<<" sig "<<significant<<" at "<<hoptimize->GetXaxis()->GetBinUpEdge(ix)<<" purity "<<valsig/(valsig+valbkg)<<endl;
    }
