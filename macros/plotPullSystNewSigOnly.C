void plotPullSystNewSigOnly()
{
    ...
    for (Int_t itoy=0; itoy<NTOYFIT; ++itoy) {
        pFile[itoy] = new TFile(Form("../outputs/one/fittoy_result_%d.root",itoy+1));
    }
    //fit result
    ...
    TFile *popfile = new TFile("../outputs/checkpull_"+subSaveName+".root","RECREATE");
