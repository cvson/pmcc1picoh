...
const int markerStyleBin[] = {20, 21, 22, 24, 25/*, 26,27,28,29*/};   
	...
	TLatex* tlx=new TLatex(0.42, 0.95,Form("%s (data) ",nameSample_long[isam]));
        ...
        tlx->Draw();
        gPad->Print(Form("plots/detresponse_fvtimenosignewfv_dt_3bptheta_timing_sam%d.eps",isam));
	...
	tlx->Draw();
        gPad->Print(Form("plots/detresponse_fvtimenosignewfv_dt_3bptheta_fv_sam%d.eps",isam));
	...
	tlx=new TLatex(0.42, 0.95,Form("%s (MC) ",nameSample_long[isam]));
        tlx->SetNDC(kTRUE); // <- use NDC coordinate
        tlx->SetTextSize(0.05);
        gPad->Print(Form("plots/detresponse_fvtimenosignewfv_mc_3bptheta_fv_sam%d.eps",isam));
      	//by bin
...
*/
	...
	tlx=new TLatex(0.42, 0.95,Form("%s (data) ",nameSample_long[isam]));
        ...
        tlx->SetTextSize(0.05);
	tlx->Draw();
        gPad->Print(Form("plots/detresponse_fvtimenosignewfv_dt_3bptheta_veto_sam%d.eps",isam));
	...
	tlx=new TLatex(0.42, 0.95,Form("%s (MC) ",nameSample_long[isam]));
        ...
        tlx->SetTextSize(0.05);
	tlx->Draw();
        gPad->Print(Form("plots/detresponse_fvtimenosignewfv_mc_3bptheta_veto_sam%d.eps",isam));
...
leg0bin->AddEntry(h2,"Sideband","l");
 ...
 new TCanvas;
	...
	pgrccqesub_fv[1][0]->GetYaxis()->SetTitle("Data-MC difference");
        ...
        tlx->Draw();
        gPad->Modified();
        gPad->Print(Form("plots/detresponse_fvtimenosignewfv_3bptheta_sigvssb_%s_compAllBin.eps", "fv"));
...
new TCanvas;
    ...
    tlx->Draw();
    gPad->Modified();
    gPad->Print(Form("plots/detresponse_fvtimenosignewfv_3bptheta_sigvssb_%s_compAllBin.eps", "veto"));
...
/////////////////////////////
    ...
    leg0->Draw("same");
    gPad->Modified();
    gPad->Print("plots/detresponse_fvtimenosignewfv_alldet_3bptheta_fv_compSample.eps");
    ...
    leg0->Draw("same");
    gPad->Modified();
    gPad->Print("plots/detresponse_fvtimenosignewfv_alldet_3bptheta_veto_compSample.eps");
    ...
    leg0->Draw("same");
    gPad->Modified();
    gPad->Print("plots/detresponse_fvtimenosignewfv_alldet_3bptheta_fvveto_compSample.eps");
