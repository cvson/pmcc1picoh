...
void t2kpre_plotDetResponse_hiteff_1bin_nosb(){
   ...
   pgrccqe_bkg[isam] = new TGraph(NFILE,xsigma,nevtbyBin_bkg);
    ...
    gPad->Print(Form("plots/detsyst_hiteff_1bin_nosb_sig_sam%d.eps",isam));
    cout<<"sam "<<isam<<"sig frac "<<res1<<endl;
    titleStyle(pgrccqe_bkg[isam]);
    new TCanvas;
    float res2 = Draw(pgrccqe_bkg[isam], "Background");
    gPad->Print(Form("plots/detsyst_hiteff_1bin_nosb_bkg_sam%d.eps",isam));
