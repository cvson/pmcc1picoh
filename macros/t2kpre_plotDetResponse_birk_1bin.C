{
    ...
    pFile[1] = new TFile("../inputs/detresponsefunction_nominal.root");
    pFile[2] = new TFile("../inputs/detresponsefunction_birks_shiftplus.root");
    pFile[3] = new TFile("../inputs/detresponsefunction_birks_shiftminus.root");
    ...
    for (int isam=0; isam<NSAMPLE; ++isam) {
    	TString savename = Form("detsyst_birks_comp_sam%d",isam);
 ...
 //make ratio and uncertainty
    ...
    TFile *popfile = new TFile("../inputs/detresponse_birks_1bptheta_uncertainty.root","RECREATE");
    ...
    for (int isam=0; isam<NSAMPLE; ++isam) {
        ...
        pUncertainty[isam]->Write(Form("detresponse_birks_1bptheta_uncertainty_sam%d",isam));
      //for signal
	for (Int_t ibin=0; ibin<NBINCCQE; ibin++) {
	...
	pUncertainty_sig[isam]->Write(Form("detresponse_birks_sig_1bptheta_uncertainty_sam%d",isam));
	//for background
      ...
      for (Int_t ibin=0; ibin<NBINCCQE; ibin++) {
        ...
        pUncertainty_bkg[isam]->Write(Form("detresponse_birks_bkg_1bptheta_uncertainty_sam%d",isam));
      }
