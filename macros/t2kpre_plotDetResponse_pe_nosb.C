float Draw(TGraph *g, TString fname){
    ...
    g->GetXaxis()->SetTitle("Light Yield (P.E.) Variation (%)");
...
void t2kpre_plotDetResponse_pe_nosb(){
...
for (int isam=0; isam<NSAMPLE; ++isam) {
        for (int ifile=0; ifile < NFILE; ++ifile) {
      nevtbyBin[ifile] = hbinccqe[isam][ifile]->GetSumOfWeights()/hbinccqe[isam][4]->GetSumOfWeights();
            nevtbyBin_bkg[ifile] = hbinccqe_bkg[isam][ifile]->GetSumOfWeights()/hbinccqe_bkg[isam][4]->GetSumOfWeights();
   ...
   pgrccqe_bkg[isam] = new TGraph(NFILE,xsigma,nevtbyBin_bkg);
    ...
    gPad->Print(Form("plots/detsyst_pe_nosb_sig_sam%d.eps",isam));
    cout<<"sam "<<isam<<"sig frac "<<res1<<endl;
    titleStyle(pgrccqe_bkg[isam]);
    new TCanvas;
    float res2 = Draw(pgrccqe_bkg[isam], "Background");
    gPad->Print(Form("plots/detsyst_pe_nosb_bkg_sam%d.eps",isam));
