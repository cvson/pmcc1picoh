...
void t2kpre_T2KReWeightGetCCcohRev_ana1x_plusmin(int nw = 0){
  ...
  ofstream output(Output);
    ...
    string fccqe  = "/home/cvson/cc1picoh/FIT/datafsicornorange/merged_ccqe_forResponseFunction.root";
     // string fccqe  = "/home/cvson/cc1picoh/FIT/datafsipionFS/merged_ccqe_forResponseFunction_pionFS.root";//newfile
  TTree* tree_nominal ;
  TTree* tree_ccqe;
  ...
  tree_nominal -> Project("fHint" , draw, Form("(%s)*weights.At(%d)*(weights.At(%d)>-100&&weights.At(%d)<100)*(inttype==16)*fv(xnu,ynu,znu)", select,nw,nw,nw ) );
  tree_ccqe -> Project("fHdet_qe" , draw, Form("(%s)*reweight[%d]*(reweight[%d]>-100&&reweight[%d]<100)*(%s&&inttype==16)", select,nw,nw,nw,cccohmva ) );
  tree_ccqe -> Project("fHdet_nqe" , draw, Form("(%s)*reweight[%d]*(reweight[%d]>-100&&reweight[%d]<100)*(%s&&inttype!=16)", select,nw,nw,nw,cccohmva ) );
