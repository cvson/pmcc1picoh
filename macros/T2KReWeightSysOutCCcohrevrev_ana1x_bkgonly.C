...
void T2KReWeightSysOutCCcohrevrev_ana1x_bkgonly(){
  int nw = 0;
  char FileName[300];
 // ofstream ofile("output_ccqe_revrev.txt");
//ofstream ofile("output_cccoh_revrev_ana1x_data.txt");  
ofstream ofile("output_cccoh_revrev_ana1x_bkgonly.txt");
float syst[2],tot[2];
  ...
  for(int i=0;i<25;i++){
    for(int j=0;j<7;j++){
      ...
      sprintf(FileName,"/home/cvson/cc1picoh/FIT/versionfsicombgenieana1x/macros/t2kreweight_ana1x/nevent%d_cccoh.txt",i*7+j);
