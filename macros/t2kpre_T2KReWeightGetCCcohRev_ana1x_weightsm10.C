...
void t2kpre_T2KReWeightGetCCcohRev_ana1x_weightsm10(int nw = 0){
  ...
  tree_nominal -> Project("fHint" , draw, Form("(%s)*weights.At(%d)*(weights.At(%d)>0&&weights.At(%d)<5)*(inttype==16)*fv(xnu,ynu,znu)", select,nw,nw,nw ) );
  tree_ccqe -> Project("fHdet_qe" , draw, Form("(%s)*reweight[%d]*(reweight[%d]>0&&reweight[%d]<5)*(%s&&inttype==16)", select,nw,nw,nw,cccohmva ) );
  tree_ccqe -> Project("fHdet_nqe" , draw, Form("(%s)*reweight[%d]*(reweight[%d]>0&&reweight[%d]<5)*(%s&&inttype!=16)", select,nw,nw,nw,cccohmva ) );
