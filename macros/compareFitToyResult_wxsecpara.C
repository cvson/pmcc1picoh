{
    ...
    TString subbin = "newbin2_wxsecpara";
    ...
    for (Int_t isam = 0; isam<NFIT2COMP; ++isam){
        if (isam==0){
            ...
            hxsecpara[isam]->GetYaxis()->SetRangeUser(-1.0,1.0);
    ...
    leg0->Draw();
    gPad->Print("plots/compfittoyresult_xsecpara_"+subbin+".eps");
    ...
    for (Int_t isam = 0; isam<NFIT2COMP; ++isam){
        if (isam==0){
            ...
            hxsecpara_bfit[isam]->GetYaxis()->SetRangeUser(-0.0001,0.0001);
    ...
    leg0->SetX2(0.45);
    leg0->Draw();
    gPad->Print("plots/compfittoyresult_xsecpara_bfit_"+subbin+".eps");
    //
    new TCanvas;
    for (Int_t isam = 0; isam<NFIT2COMP; ++isam){
        if (isam==0){
            hxsecpara_err[isam]->SetTitle("");
            hxsecpara_err[isam]->GetYaxis()->SetRangeUser(0.0,0.7);
    ...
    leg0->Draw();
    gPad->Print("plots/compfittoyresult_xsecpara_err_"+subbin+".eps");
