{
    ...
    double ymaxgraph[NSYS]={0.2,0.5,0.5,0.2,0.2};
    ...
    for (Int_t isys=0; isys<NSYS; ++isys) {
        for (Int_t isam=0; isam<NSAMPLE; ++isam) {
            ...
            //fake color here
            if(isam>3)icolor = TColor::GetColor(colorcode[isam-2]);
    ...
    TFile *popfile = new TFile("../inputs/detectorAll_covariance_matrix_tmp.root","RECREATE");
    ...
    for (int isam=0;isam<NSAMPLE;isam++){
        if(isam!=2 && isam!=3)leg0->AddEntry(pUncertainty[0][isam], nameSample_long[isam],"pl");
    }
    leg0->SetFillStyle(0);
    ...
    for (Int_t isys=0; isys<NSYS; ++isys) {
        ...
        for (Int_t isam=1; isam<NSAMPLE; ++isam){
            if(isam!=2 && isam!=3) pUncertainty[isys][isam]->Draw("PL same");
        }
        ...
        leg0->Draw("same");
        gPad->Modified();
        gPad->Print(Form("plots/alldet_uncertainty_%s_compSample_official.eps",subname[isys]));
    }
    ...
    pUncertaintyComb[0]->GetYaxis()->SetRangeUser(0,0.5);
    pUncertaintyComb[0]->Draw("APL");
    for (Int_t isam=1; isam<NSAMPLE; ++isam){
        if(isam!=2 && isam!=3)pUncertaintyComb[isam]->Draw("PL same");
    }
    ...
    leg0->Draw("same");
    gPad->Modified();
    gPad->Print("plots/alldet_alluncertainty_compSample_official.eps");
