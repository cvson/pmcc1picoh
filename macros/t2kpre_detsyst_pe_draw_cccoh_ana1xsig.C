...
float Draw(TGraphErrors *g){
  ...
  g->GetXaxis()->SetTitle("Variation of P.E. [%]");
...
void t2kpre_detsyst_pe_draw_cccoh_ana1xsig(){
  ...
  sprintf(FileName_qe1,"detsyst/pe_ana1xsig_nevent_cccoh1_qe.txt");
  sprintf(FileName_nqe1,"detsyst/pe_ana1xsig_nevent_cccoh1_nqe.txt");
  sprintf(OutFile1,"detsyst/pe_ana1xsig_slope_cccoh1.txt");
  //sprintf(FileName_qe2,"detsyst/pe_ana1xsig_nevent_cccoh2_qe.txt");
  //sprintf(FileName_nqe2,"detsyst/pe_ana1xsig_nevent_cccoh2_nqe.txt");
  ...
  int nomi=4;
  for(int i=0;i<nevt;i++){
    y[0][i]=(qe1[nomi]*result_qemva+nqe1[nomi]-nqe1[i])/qe1[i]/result_qemva;
    //y[1][i]=(qe2[noomi]*result_qemva+nqe2[nomi]-nqe2[i])/qe2[i];
  ...
  output1<<res1<<endl;
  titleStyle(g1);
 // c1->Print("detsyst/pe_ana1xsig_graph.eps");
 c1->Print("detsyst/pe_ana1xsig_graph_rat04.eps"); 
 //c1->cd(2);
  //float res2 = Draw(g2);
  //output2<<res2<<endl;
}
