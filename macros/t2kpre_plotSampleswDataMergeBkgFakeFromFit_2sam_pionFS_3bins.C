{
    ...
    Bool_t isNEUTfit = false;
    TFile *pfile = new TFile("../outputs/two/fittoy_result_1xseconlyhi.root");
    string subsavename = isNEUTfit? "xseconly_twohi_neut":"xseconly_twohi_genie";
    
/*	TFile *pfile = new TFile("../outputs/three/fittoy_result_1xseconlylo.root");
    string subsavename = isNEUTfit? "xseconly_threelo_neut":"xseconly_threelo_genie";
*/
    string tag = "nominal";
    string tagfit = "fit";
    const int nreac = 9;//10;
    const int nsample = 2;
    const int sampleindex[] = {0,1,2};
    const char *m_name[] = {"SIG",/* "CRI","CRII",*/"CRIII"};
    const char *m_namelong[] = {"Signal", "sideband"/*,"Control I","Control II"*/};
    ...
    for(int isam=0;isam<nsample;isam++){
        hccqebin_rec_data[isam]= (TH1D*)pfile->Get(Form("evhist_sam%d_iter0_data",sampleindex[isam]));
    ...
    if(isNEUTfit) leg0->AddEntry(hccqebin_rec_data[0], "NEUT MC","lp");
	else leg0->AddEntry(hccqebin_rec_data[0], "GENIE MC","lp");
    for (int ireac=0;ireac<nreac;ireac++){
    ...
    for(int isam=0;isam<nsample;isam++){
        ...
        fHstach_ccqebin_rec[isam]->GetXaxis()->SetTitle("Reconstructed p_{#mu}-cos(#theta_{#mu}) bins");
        ...
        leg0->Draw();
        gPad->Modified();
        gPad->Print(Form("plots/allfakefit_3bptheta_hdatamc_ccqebin_rec_%s_sam%d.eps",subsavename.c_str(),isam));
        
        //fit
        new TCanvas;
        fHstach_ccqebin_rec_fit[isam]->Draw();
        fHstach_ccqebin_rec_fit[isam]->GetXaxis()->SetTitle("Reconstructed p_{#mu}-cos(#theta_{#mu}) bins");
        ...
        leg0->Draw();
        gPad->Modified();
        gPad->Print(Form("plots/allfakefit_3bptheta_hdatamc_ccqebin_rec_%s_fit_sam%d.eps",subsavename.c_str(),isam));
    }
