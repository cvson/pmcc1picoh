...
"#D55E00"
    ...
    for (int isam=0; isam<2; ++isam) {
   	...
   	pgrccqe[isam][ibin]->SetMarkerStyle(imarker);
        ...
        gPad->Print(Form("plots/detresponse_noise_3bptheta_all_sam%d_compSyst.eps",isam));
    ...
    for (Int_t ibin=0; ibin<NBINCCQE; ibin++) {
        ...
        leg0->Draw("same");
        gPad->Modified();
        gPad->Print(Form("plots/detresponse_noise_3bptheta_bin%d_compSyst.eps",ibin));
...
leg0bin->Draw("same");
      gPad->Modified();
gPad->Print("plots/detresponse_noise_3bptheta_sig_all_sigvssb.eps");
    ...
    TFile *popfile = new TFile("../inputs/detresponse_noise_3bptheta_uncertainty.root","RECREATE");
    for (int isam=0; isam<NSAMPLE; ++isam) {
        for (Int_t ibin=0; ibin<NBINCCQE; ibin++) {
        ...
        pUncertainty[isam]->Write(Form("detresponse_noise_3bptheta_uncertainty_sam%d",isam));
    }
    popfile->Write();
    popfile->Close();
