{
    ...
    TFile *pfile = new TFile("../outputs/five/fittoy_result_1.root");
    string tag = "nominal";
    string tagfit = "fit";
    const int nreac = 9;//10;
    const int nsample = 5;
    const char *m_name[] = {"SIG", "CRI", "CRIIA", "CRIIC", "More2trk"};
    const char *m_namelong[] = {"Signal", "Control I", "Control IIA", "Control IIC", "> two tracks"};
    ...
    for(int isam=0;isam<nsample;isam++){
        ...
        leg0->Draw();
        gPad->Modified();
        gPad->Print(Form("plots/5samfakefit_newbin_mergebkg_hdatamc_ccqebin_rec_sam%d_newbin2.eps",isam));
        ...
        leg0->Draw();
        gPad->Modified();
        gPad->Print(Form("plots/5samfakefit_newbin_mergebkg_hdatamc_ccqebin_rec_fit_sam%d_newbin2.eps",isam));
    }
