...
"#D55E00"
    ...
    for (int isam=0; isam<NSAMPLE; ++isam) {
        ...
        gPad->Print(Form("plots/detresponse_hiteff_1bptheta_all_sam%d_compSyst.eps",isam));
    ...
    for (Int_t ibin=0; ibin<NBINCCQE; ibin++) {
        ...
        leg0->Draw("same");
        gPad->Modified();
        gPad->Print(Form("plots/detresponse_hiteff_1bptheta_bin%d_compSyst.eps",ibin));
   ...
   pgrccqe[1][ibin]->SetLineStyle(7);
    ...
    pgrccq[2][ibin]->SetLineColor(2);
 pgrccqe[3][ibin]->SetLineColor(4);
   //Get Covariance
...
pgrccqe[2][ibin]->Draw("PL same");
pgrccqe[3][ibin]->Draw("PL same");
}
leg0bin->Draw("same");
      gPad->Modified();
gPad->Print("plots/detresponse_hiteff_1bptheta_sig_all_sigvssb.eps");
...
leg0bin->Draw("same");
      gPad->Modified();
gPad->Print("plots/detresponse_hiteff_1bptheta_bkg_all_sigvssb.eps");    
*/
    //fitting
    ...
    TFile *popfile = new TFile("../inputs/detresponse_hiteff_1bptheta_uncertainty.root","RECREATE");
    for (int isam=0; isam<NSAMPLE; ++isam) {
        for (Int_t ibin=0; ibin<NBINCCQE; ibin++) {
        ...
        pUncertainty[isam]->Write(Form("detresponse_hiteff_1bptheta_uncertainty_sam%d",isam));
    }
    popfile->Write();
    popfile->Close();
