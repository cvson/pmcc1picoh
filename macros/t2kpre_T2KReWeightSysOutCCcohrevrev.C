...
void t2kpre_T2KReWeightSysOutCCcohrevrev(){
  int nw = 0;
  char FileName[300];
 // ofstream ofile("output_ccqe_revrev.txt");
//ofstream ofile("output_cccoh_revrev_nomcheck.txt");  
//ofstream ofile("output_cccoh_revrev_nomcheck_kikawa.txt"); 
ofstream ofile("output_cccoh_revrev_nom_v1r19allmepiless02fixpotscale.txt");
float syst[2],tot[2];
  ...
  for(int i=0;i<25;i++){
    for(int j=0;j<7;j++){
      ...
      //sprintf(FileName,"/home/cvson/cc1picoh/FIT/versionfsicombgenieana1xpionFS2trksb100mev/macros/t2kreweight_nomcheck_kikawa/nevent%d_cccoh.txt",i*7+j);
      ...
      //sprintf(FileName,"/export/scraid5/data2/home_recover/kikawa/macros/t2kreweight_cccoh/nevent%d_cccoh.txt",i*7+j);
