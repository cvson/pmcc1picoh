{
    ...
    TFile *pfileFIT = new TFile("../outputs/two/fittoy_result_1nodet.root");
    ...
    TString subname = "ana1x_genievsneutvsfit_nodetpiless02cor_"; 
    TH1D *hmcneut_sig;
    TH1D *hmcgenie_sig;
    TH1D *hfit_sig;
   ...
   //check the ratio between NEUT and GENIE
    ...
    plot3histdata(hfit_sig,"Fit result",hmcneut_sig,"NEUT MC",hmcgenie_sig,"GENIE MC",subname+"sig",0.4,0.65);
    ...
    plot3histdata(hfit_bkg,"Fit result",hmcneut_bkg,"NEUT MC",hmcgenie_bkg,"GENIE MC",subname+"bkg",0.65,0.2);
}
