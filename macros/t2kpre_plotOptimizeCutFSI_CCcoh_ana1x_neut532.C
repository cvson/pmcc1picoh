{
//THIS is modified 
//for optimize of analysis one, need sample included the cut on 50MeV
    ...
    TFile *pfile = new TFile("../outputs/basicHisto_neut5d3d2_20170529_sam167.root");
    //TFile *pfile = new TFile("../outputs/basicHisto_genie_20170529_sam167.root");
    //TFile *pfile = new TFile("../outputs/basicHisto_neut5d1d4d2_20170529_sam167.root");
    ...
    const int NTOPOLOGY = 1;
    const int ithtopo = 167;
    ...
    for (Int_t itopo=0; itopo<NTOPOLOGY; ++itopo) {
        ...
        for (Int_t iint=0; iint<NINTERACTION; ++iint) {
            hmcen[itopo][iint] = (TH1F*)pfile->Get(Form("hmc_topo%d_var%d_int%d",ithtopo,thvariable,iint));
            hmcen[itopo][iint]->Rebin(nrebin);
            hmcen[itopo][iint]->SetLineWidth(3);
            
        }
        hdata[itopo] = (TH1F*)pfile->Get(Form("hdata_topo%d_var%d",ithtopo,0));
    ...
    //NEED to change pre-selected sample
    TH1F* hsig = (TH1F*)hmcen[0][1]->Clone("hsignal");
    TH1F* hbkg = (TH1F*)hmcen[0][2]->Clone("hbackground");
    for (Int_t iint=3; iint<NINTERACTION; ++iint) {
        hbkg->Add(hmcen[0][iint]);
    }
    ...
    hoptimize->Draw("hist");
    gPad->Print("plots/neut532_ana1x_"+varname+"_opt.eps");
    //hbkg->SetLineColor(2);
    //hbkg->Draw("hist same");
