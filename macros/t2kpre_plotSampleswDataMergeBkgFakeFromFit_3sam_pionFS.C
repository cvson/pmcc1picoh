{
    ...
    //string subsavename = "nodet";
    string subsavename = "2sb_xseconly";
    string tag = "nominal";
    string tagfit = "fit";
    const int nreac = 9;//10;
    const int nsample = 3;
    const int sampleindex[] = {0,1,2};
    const char *m_name[] = {"SIG", "CRI","CRII"};
    const char *m_namelong[] = {"Signal", "Sideband","Control"};
    ...
    for(int isam=0;isam<nsample;isam++){
        ...
        leg0->Draw();
        gPad->Modified();
        gPad->Print(Form("plots/allfakefit_5bptheta_hdatamc_ccqebin_rec_%s_sam%d.eps",subsavename.c_str(),isam));
        ...
        leg0->Draw();
        gPad->Modified();
        gPad->Print(Form("plots/allfakefit_5bptheta_hdatamc_ccqebin_rec_%s_fit_sam%d.eps",subsavename.c_str(),isam));
    }
