{
    ...
    TFile *pfile = new TFile("../outputs/six/fittoy_result_1.root");
    string tag = "nominal";
    string tagfit = "fit";
    const int nreac = 9;//10;
    const int nsample = 6;
    const int sampleindex[] = {0,1,4,5,2,3};
    ...
    for(int isam=0;isam<nsample;isam++){
        ...
        leg0->Draw();
        gPad->Modified();
        gPad->Print(Form("plots/allfakefit_newbin_mergebkg_hdatamc_ccqebin_rec_sam%d_newbin2.eps",isam));
        ...
        leg0->Draw();
        gPad->Modified();
        gPad->Print(Form("plots/allfakefit_newbin_mergebkg_hdatamc_ccqebin_rec_fit_sam%d_newbin2.eps",isam));
    }
