{
 ...
 //make ratio and uncertainty
    ...
    TFile *popfile = new TFile("../inputs/detresponse_pevar_5bptheta_uncertainty.root","RECREATE");
    ...
    for (int isam=0; isam<NSAMPLE; ++isam) {
        ...
        pUncertainty[isam]->Write(Form("detresponse_pevar_5bptheta_uncertainty_sam%d",isam));
      //for signal
	for (Int_t ibin=0; ibin<NBINCCQE; ibin++) {
	...
	pUncertainty_sig[isam]->Write(Form("detresponse_pevar_sig_5bptheta_uncertainty_sam%d",isam));
	//for background
      ...
      for (Int_t ibin=0; ibin<NBINCCQE; ibin++) {
        ...
        pUncertainty_bkg[isam]->Write(Form("detresponse_pevar_bkg_5bptheta_uncertainty_sam%d",isam));
      }
