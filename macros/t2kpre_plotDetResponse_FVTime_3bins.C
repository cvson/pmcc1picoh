...
const int markerStyleBin[] = {20, 21, 22, 24, 25/*, 26,27,28,29*/};   
	...
	TLatex* tlx=new TLatex(0.42, 0.95,Form("%s ",nameSample_long[isam]));
        ...
        tlx->Draw();
        gPad->Print(Form("plots/detresponse_fvtimenosig_dt_3bptheta_timing_sam%d.eps",isam));
	...
	tlx->Draw();
        gPad->Print(Form("plots/detresponse_fvtimenosig_dt_3bptheta_fv_sam%d.eps",isam));
	...
	hmc_fv[isam][0]->Draw();
        ...
        gPad->Print(Form("plots/detresponse_fvtimenosig_mc_3bptheta_fv_sam%d.eps",isam));
      	//by bin
...
*/
	...
	tlx->Draw();
        gPad->Print(Form("plots/detresponse_fvtimenosig_dt_3bptheta_veto_sam%d.eps",isam));
	...
	tlx->Draw();
        gPad->Print(Form("plots/detresponse_fvtimenosig_mc_3bptheta_veto_sam%d.eps",isam));
...
leg0bin->AddEntry(h2,"Sideband","l");
 ...
 new TCanvas;
	...
	pgrccqesub_fv[1][0]->GetYaxis()->SetTitle("Data-MC difference");
        ...
        tlx->Draw();
        gPad->Modified();
        gPad->Print(Form("plots/detresponse_fvtimenosig_3bptheta_sigvssb_%s_compAllBin.eps", "fv"));
...
new TCanvas;
    ...
    tlx->Draw();
    gPad->Modified();
    gPad->Print(Form("plots/detresponse_fvtimenosig_3bptheta_sigvssb_%s_compAllBin.eps", "veto"));
...
/////////////////////////////
    ...
    leg0->Draw("same");
    gPad->Modified();
    gPad->Print("plots/detresponse_fvtimenosig_alldet_3bptheta_fv_compSample.eps");
    ...
    leg0->Draw("same");
    gPad->Modified();
    gPad->Print("plots/detresponse_fvtimenosig_alldet_3bptheta_veto_compSample.eps");
    ...
    leg0->Draw("same");
    gPad->Modified();
    gPad->Print("plots/detresponse_fvtimenosig_alldet_3bptheta_fvveto_compSample.eps");
