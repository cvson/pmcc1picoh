{
    ...
    TFile *pfile = new TFile("../outputs/basicHisto_datamcfsi_finalcomb_sam4testv2_addnue_morepid_ana1x_pionFSnewBD_50mevff_antipid.root");
    ...
    //sum of background
    //7 is after pid<0.0
    TH1F* hsig = (TH1F*)hmcen[7][3]->Clone("hsignal");
    hsig->Add(hmcen[7][1]);//coherent is cc1pi too
    TH1F* hbkg = (TH1F*)hmcen[7][4]->Clone("hbackground");
    for (Int_t iint=2; iint<NINTERACTION; ++iint) {
       if(iint!=4 && iint!=3) hbkg->Add(hmcen[7][iint]);
    }
    //optimization
    ...
    Double_t valsig;
    Double_t valbkg;
    Double_t significantRaw = valsigTot/(valbkgTot+valsigTot);/*valsigTot/valbkgTot;*/valsigTot/sqrt(valbkgTot);
    ...
    for (int ix = nx; ix >= 1; ix--){
        ...
        significant = valsig/(valbkg+valsig);///*valsig/valbkg;*/valsig/sqrt(valbkg);
        ...
        cout<<"raw "<<significantRaw<<" sig "<<significant<<" at "<<hoptimize->GetXaxis()->GetBinUpEdge(ix)<<" purity "<<valsig/(valbkg+valsig)<<endl;
    }
    cout<<"raw "<<significantRaw<<"max "<<significantMax<<" at "<<hoptimize->GetXaxis()->GetBinUpEdge(binxwMax)<<endl;
    new TCanvas;
    hoptimize->GetXaxis()->SetTitle("Cut values on discriminant parameter (RES)");
    ...
    hoptimize->Draw("hist");
    gPad->Print("plots/antipid_"+varname+"_opt.eps");
    //hbkg->SetLineColor(2);
    //hbkg->Draw("hist same");
