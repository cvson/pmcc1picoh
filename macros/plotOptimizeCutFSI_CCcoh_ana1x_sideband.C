{
    ...
    TH1F* hsig = (TH1F*)hmcen[4][1]->Clone("hsignal");
    TH1F* hbkg = (TH1F*)hmcen[4][2]->Clone("hbackground");
    for (Int_t iint=3; iint<NINTERACTION; ++iint) {
        hbkg->Add(hmcen[4][iint]);
    }
    ...
    Double_t valsig;
    Double_t valbkg;
    Double_t significantRaw = 0.03;//valsigTot/sqrt(valbkgTot);
    ...
    for (int ix = nx; ix >= 1; ix--){
        ...
        significant = valsig/(valsig+valbkg);//valsig/sqrt(valbkg);
    ...
    /*new TCanvas;
    hoptimize->GetXaxis()->SetTitle("Cut values on discriminant parameter (COH)");
