...
"#D55E00"
   ...
   const int markerStyleBin[] = {20, 21, 22, 24, 25/*, 26,27,28,29*/}; 
    ...
    for (int isam=0; isam<NSAMPLE; ++isam) {
        ...
        tlx->Draw();
        gPad->Print(Form("plots/detresponse_reco_1bptheta_hbinccqe_mc_sam%d_compAll.eps",isam));
        ...
        tlx->Draw();
        gPad->Print(Form("plots/detresponse_reco_1bptheta_hbinccqe_data_sam%d_compAll.eps",isam));
    ...
    for (Int_t isys=0; isys<NSYSRECO; ++isys) {
        for (Int_t ibin=0; ibin<NBINCCQE; ++ibin) {
            ...
            leg0->Draw("same");
            gPad->Modified();
            gPad->Print(Form("plots/detresponse_reco_1bptheta_%s_bin%d_compSyst.eps",namesys[isys],ibin));
        }
    }
    ...
    for (Int_t isys=0; isys<NSYSRECO; ++isys) {
        for (Int_t isam=0; isam<NSAMPLE; ++isam) {
            ...
            tlx->Draw();
            gPad->Modified();
            gPad->Print(Form("plots/detresponse_reco_1bptheta_%s_%s_compAllBin.eps",nameSample[isam], namesys[isys]));
          */
          }
    }
...
for (Int_t isys=0; isys<NSYSRECO; ++isys) {
	...
	TLatex* tlx=new TLatex(0.42, 0.95,Form("Syst: %s",namesyslong[isys]));
        ...
        tlx->Draw();
        gPad->Modified();
        gPad->Print(Form("plots/detresponse_reco_1bptheta_sigvssb_%s_compAllBin.eps", namesys[isys]));
}
    ...
    TFile *popfile = new TFile("../inputs/detresponse_reco_1bptheta_uncertainty.root","RECREATE");
    ...
    for (int isam=0; isam<NSAMPLE; ++isam) {
        ...
        for (Int_t isys=0; isys<NSYSRECO; ++isys) {
            ...
            pUncertainty[isys][isam]->Write(Form("detresponse_reco_1bptheta_uncertainty_sam%d_sys%d",isam,isys));
        }
        //make square root
        for (Int_t ibin=0; ibin<NBINCCQE; ibin++) {
        ...
        pUncertaintyComb[isam]->Write(Form("detresponse_reco_1bpthetaall_uncertainty_sam%d",isam));
    }
