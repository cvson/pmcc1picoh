{
    ...
    TFile *pfile = new TFile("../outputs/mcdata_sample_newbin2.root");
    string tag = "nominal";
    const int nreac = 9;//10;
    const int nsample = 12;
    const char *m_name[] = {"SIG", "CRI", "CRIIALoQsq", "CRIIAHiQsq","CRIIB", "CRIICHiResPID", "CRIICLoResPID", "M2TRKHiResPID","M2TRKLoResPID", "M2TRKAllElse", "ONETRKLoQsq","ONETRKHiQsq"};
    const char *m_namelong[] = {"SIG", "CRI", "CRIIA LowQ^{2}", "CRIIA High Q^{2}","CRIIB", "CRIIC High PID_{res}", "CRIIC Low PID_{res}", ">2-trk high PID_{cccoh}",">2-trk low vertex act.", ">2 track AllElse", "one-trk low Q^{2}","one-trk high Q^{2}"};
    ...
    for(int isam=0;isam<nsample;isam++){
        ...
        tlx->Draw();
        gPad->Modified();
        gPad->Print(Form("plots/newbin2_mergebkg_hstach_enu_true_sam%d.eps",isam));
        ...
        tlx->Draw();
        gPad->Modified();
        gPad->Print(Form("plots/newbin2_mergebkg_hstach_enu_rec_sam%d.eps",isam));
        ...
        tlx->Draw();
        gPad->Modified();
        gPad->Print(Form("plots/newbin2_mergebkg_hstach_pmu_rec_sam%d.eps",isam));
        ...
        tlx->Draw();
        gPad->Modified();
        gPad->Print(Form("plots/newbin2_mergebkg_hstach_costh_rec_sam%d.eps",isam));
        ...
        tlx->Draw();
        gPad->Modified();
        gPad->Print(Form("plots/newbin2_mergebkg_hstach_pmu_true_sam%d.eps",isam));
        ...
        tlx->Draw();
        gPad->Modified();
        gPad->Print(Form("plots/newbin2_mergebkg_hstach_costh_true_sam%d.eps",isam));
        ...
        tlx->Draw();
        gPad->Modified();
        gPad->Print(Form("plots/newbin2_mergebkg_hstach_ccqebin_true_sam%d.eps",isam));
        ...
        tlx->Draw();
        gPad->Modified();
        gPad->Print(Form("plots/newbin2_mergebkg_hstach_ccqebin_rec_sam%d.eps",isam));
        */
        new TCanvas;
        fHstach_ccqebin_rec[isam]->Draw();
        fHstach_ccqebin_rec[isam]->GetXaxis()->SetTitle("Reconstructed p-cos(#theta) bins");
        ...
        if(isam>0)hccqebin_rec_data[isam]->Draw("ep same");
        ...
        if(isam>0)leg0->Draw();
            ...
            leg0nodt->SetY1NDC(0.48);
            leg0nodt->SetY2NDC(0.91);
            leg0nodt->Draw();
        }
        gPad->Modified();
        gPad->Print(Form("plots/newbin2_mergebkg_hdatamc_ccqebin_rec_sam%d.eps",isam));
